...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('repoguardian.fixIssue', showFixWizard));
    // NEW: Register Scan Summary command
    context.subscriptions.push(vscode.commands.registerCommand('repoguardian.openScanSummary', openScanSummary));
    // NEW: Register hover provider for diagnostics
    context.subscriptions.push(vscode.languages.registerHoverProvider('*', new RepoGuardianHoverProvider()));
    // NEW: Register code action provider for quick fixes
    context.subscriptions.push(vscode.languages.registerCodeActionsProvider('*', new RepoGuardianCodeActionProvider(), {
    ...
    if (workspacePath) {
        ...
        (0, reports_1.cleanupOldReports)(workspacePath, retentionDays);
    }
}
function deactivate() {
...
async function runSecurityScan() {
    ...
    try {
        ...
        const report = await vscode.window.withProgress({
            ...
            return (0, scanner_1.scanWorkspace)({
                workspacePath,
                maxFileSize,
                ignorePaths
            });
        ...
        if (report.detectionsFound === 0) {
            ...
            if (enableNotifications) {
                vscode.window.showInformationMessage(`✅ RepoGuardian: Clean - No issues in ${report.filesScanned} files`);
            }
        }
        else {
            updateStatusBar('issues', report.detectionsFound);
            if (enableNotifications) {
                const action = await vscode.window.showWarningMessage(`⚠️ RepoGuardian: ${report.detectionsFound} issue(s) found`, 'View Summary', 'Open Problems');
 ...
 */
async function addToIgnoreList(uriOrString, line) {
    const workspacePath = getWorkspacePath();
    if (!workspacePath) {
        return;
    }
 ...
 */
async function showFixWizard(uri, diagnostic) {
    if (!uri || !diagnostic) {
 ...
 */
async function moveSecretToEnv(uri, diagnostic, workspacePath) {
    ...
    try {
        ...
        vscode.window.showInformationMessage(`Replace the secret in ${path.basename(uri.fsPath)} with: process.env.${varName}`, 'Open File').then(action => {
            if (action === 'Open File') {
 ...
 */
async function viewMaskedReport(uri, lineNumber) {
    const workspacePath = getWorkspacePath();
    if (!workspacePath) {
        return;
    }
    ...
    const detection = report.detections.find(d => d.filePath === uri.fsPath && d.line === lineNumber);
    if (!detection) {
    ...
    const panel = vscode.window.createWebviewPanel('repoguardianReport', 'Detection Details', vscode.ViewColumn.Beside, {});
    panel.webview.html = getReportWebviewContent(message);
}
/**
 * Show remediation tips based on rule type
 */
async function showRemediationTips(ruleId) {
    ...
    const panel = vscode.window.createWebviewPanel('repoguardianTips', 'Remediation Tips', vscode.ViewColumn.Beside, {});
    panel.webview.html = getReportWebviewContent(tips);
}
/**
 * Hover Provider for inline explanations
 */
class RepoGuardianHoverProvider {
 ...
 */
class RepoGuardianCodeActionProvider {
    provideCodeActions(document, range) {
        ...
        for (const diagnostic of relevantDiagnostics) {
            ...
            const fixAction = new vscode.CodeAction('🔧 Fix with RepoGuardian Wizard', vscode.CodeActionKind.QuickFix);
            fixAction.command = {
            ...
            const ignoreAction = new vscode.CodeAction('🙈 Add to .safecommit-ignore', vscode.CodeActionKind.QuickFix);
...
async function openScanSummary() {
    ...
    scanSummaryPanel = vscode.window.createWebviewPanel('repoguardianSummary', 'RepoGuardian Scan Summary', vscode.ViewColumn.One, {
        enableScripts: true,
        retainContextWhenHidden: true
    });
    // Load report data
    const report = currentReport || (0, reports_1.loadLatestReport)(workspacePath);
    scanSummaryPanel.webview.html = getScanSummaryWebviewContent(report);
    // Handle messages from webview
    scanSummaryPanel.webview.onDidReceiveMessage(async (message) => {
 ...
 */
function getScanSummaryWebviewContent(report) {
    if (!report) {
        return `
      <!DOCTYPE html>
      <html>
 ...
 */
function getReportWebviewContent(markdownContent) {
    return `
    <!DOCTYPE html>
    <html>
    <head>
