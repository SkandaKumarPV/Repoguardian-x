import { maskSecret, maskEmail, maskConnectionString, maskValue, maskSnippet, maskDetections } from '../core/masking';
import { Detection } from '../core/detector';
...
function testMaskSecret() {
  ...
  const maskedShort = maskSecret(shortSecret);
  assertEqual(maskedShort, 'a****3', 'Short secret masking (6 chars)');

  // Very short strings
  assertEqual(maskSecret('ab'), 'a*', 'Very short secret (2 chars)');
  assertEqual(maskSecret('a'), '*', 'Single char secret');
  assertEqual(maskSecret(''), '****', 'Empty string');

  // 8 character string (edge case)
  assertEqual(maskSecret('12345678'), '1234****5678', '8 character secret');
}
...
function testMaskConnectionString() {
  ...
  const masked1 = maskConnectionString(connStr1);
  ...
  const masked2 = maskConnectionString(connStr2);
...
function testMaskSnippet() {
  ...
  const masked = maskSnippet(snippet, 'aws-access-key', pattern);
...
function testMaskDetections() {
  ...
  const masked = maskDetections(detections);
...
function testFakeCredentials() {
  ...
  const masked3 = maskSecret(fakeToken);
